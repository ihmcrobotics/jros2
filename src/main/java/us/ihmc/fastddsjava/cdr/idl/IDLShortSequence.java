...
public class IDLShortSequence extends IDLSequence<IDLShortSequence> implements Iterable<Short>
{
   ...
   public int copyTo(short[] destination, int destinationOffset)
   {
      int length = size();
      if (length == 0)
         return 0;
   ...
   public int copyTo(ShortBuffer destination)
   {
      int length = size();
      if (length == 0)
         return 0;
      ...
      ShortBuffer readView = buffer.duplicate();
   ...
   /**
    * Appends all short values from the array to the end of the sequence.
   ...
   {
      return 2; // Size of short element (alignment added by caller)
   }
