...
public class ROS2Topic<T extends ROS2Message<T>>
{
   ...
   public ROS2Topic(String topicName, Class<T> topicType)
   {
      ...
      else
      {
         ...
         {
            jros2.logger().warning("Possible invalid topic name (" + topicName + "). Topics must start with a leading forward slash (/).");
         }
   ...
   public ROS2Topic<T> insert(int position, String token)
   {
      ...
      if (!tokens[0].equals(""))
      {
         jros2.logger().severe("Malformed topic name: " + topicName + ". Failed to insert token at position " + position + ".");
         return this;
      }
