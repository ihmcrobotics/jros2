package us.ihmc.fastddsjava.cdr.idl;

import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

public class IDLWStringSequenceTest
{
   private static final int INITIAL_CAPACITY = 8;

   @Test
   public void testInitialization()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      assertEquals(0, sequence.size());
      assertEquals(INITIAL_CAPACITY, sequence.capacity());
   }

   @Test
   public void testAddString()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         sequence.add("String" + i);
      }

      assertEquals(INITIAL_CAPACITY, sequence.size());
      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         assertEquals("String" + i, sequence.getAsString(i));
      }
   }

   @Test
   public void testAddStringBuilder()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         sequence.add(new StringBuilder("Builder" + i));
      }

      assertEquals(INITIAL_CAPACITY, sequence.size());
      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         assertEquals("Builder" + i, sequence.getAsString(i));
      }
   }

   @Test
   public void testAddWithAutoCreate()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         sequence.add().append("Auto").append(i);
      }

      assertEquals(INITIAL_CAPACITY, sequence.size());
      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         assertEquals("Auto" + i, sequence.getAsString(i));
      }
   }

   @Test
   public void testAddWithLength()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      StringBuilder sb = sequence.add(32);
      sb.append("TestString");

      assertEquals(1, sequence.size());
      assertEquals("TestString", sequence.getAsString(0));
      assertTrue(sequence.get(0).capacity() >= 32);
   }

   @Test
   public void testRemove()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("First");
      sequence.add("Second");
      sequence.add("Third");

      StringBuilder removed = sequence.remove();

      assertEquals("Third", removed.toString());
      assertEquals(2, sequence.size());
   }

   @Test
   public void testRemoveAtIndex()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("A");
      sequence.add("B");
      sequence.add("C");
      sequence.add("D");

      sequence.remove(1);

      assertEquals(3, sequence.size());
      assertEquals("A", sequence.getAsString(0));
      assertEquals("C", sequence.getAsString(1));
      assertEquals("D", sequence.getAsString(2));
   }

   @Test
   public void testRemoveAtBeginning()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("First");
      sequence.add("Second");
      sequence.add("Third");

      sequence.remove(0);

      assertEquals(2, sequence.size());
      assertEquals("Second", sequence.getAsString(0));
      assertEquals("Third", sequence.getAsString(1));
   }

   @Test
   public void testGet()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("TestA");
      sequence.add("TestB");

      assertEquals("TestA", sequence.get(0).toString());
      assertEquals("TestB", sequence.get(1).toString());
   }

   @Test
   public void testGetAsString()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("Hello");
      sequence.add("World");

      assertEquals("Hello", sequence.getAsString(0));
      assertEquals("World", sequence.getAsString(1));
   }

   @Test
   public void testIterator()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("One");
      sequence.add("Two");
      sequence.add("Three");

      int index = 0;
      String[] expected = {"One", "Two", "Three"};
      for (String str : sequence)
      {
         assertEquals(expected[index++], str);
      }
      assertEquals(3, index);
   }

   @Test
   public void testIteratorRemove()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("First");
      sequence.add("Second");
      sequence.add("Third");

      var iterator = sequence.iterator();
      iterator.next();
      iterator.next();
      iterator.remove();

      assertEquals(2, sequence.size());
      assertEquals("First", sequence.getAsString(0));
      assertEquals("Third", sequence.getAsString(1));
   }

   @Test
   public void testClear()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("A");
      sequence.add("B");

      int originalCapacity = sequence.capacity();
      sequence.clear();

      assertEquals(0, sequence.size());
      assertEquals(originalCapacity, sequence.capacity());
   }

   @Test
   public void testSet()
   {
      IDLWStringSequence source = new IDLWStringSequence(INITIAL_CAPACITY);
      source.add("X");
      source.add("Y");
      source.add("Z");

      IDLWStringSequence target = new IDLWStringSequence();
      target.set(source);

      assertEquals(source.size(), target.size());
      for (int i = 0; i < source.size(); ++i)
      {
         assertEquals(source.getAsString(i), target.getAsString(i));
      }

      // Verify source is unaffected
      assertEquals(3, source.size());
   }

   @Test
   public void testToStringArray()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);
      sequence.add("Alpha");
      sequence.add("Beta");
      sequence.add("Gamma");

      String[] array = sequence.toStringArray();

      assertEquals(3, array.length);
      assertEquals("Alpha", array[0]);
      assertEquals("Beta", array[1]);
      assertEquals("Gamma", array[2]);
   }

   @Test
   public void testEmptySequence()
   {
      IDLWStringSequence sequence = new IDLWStringSequence();

      assertEquals(0, sequence.capacity());
      assertEquals(0, sequence.size());
      assertDoesNotThrow(sequence::clear);
   }

   @Test
   public void testAddBeyondCapacity()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      for (int i = 0; i < INITIAL_CAPACITY; ++i)
      {
         sequence.add("Item" + i);
      }

      sequence.add("Extra");

      assertEquals(INITIAL_CAPACITY + 1, sequence.size());
      assertTrue(sequence.capacity() > INITIAL_CAPACITY);
      assertEquals("Extra", sequence.getAsString(INITIAL_CAPACITY));
   }

   @Test
   public void testUnicodeCharacters()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      // Chinese characters
      sequence.add("你好世界");
      // Japanese characters
      sequence.add("こんにちは世界");
      // Korean characters
      sequence.add("안녕하세요");
      // Arabic characters
      sequence.add("مرحبا بالعالم");
      // Emoji
      sequence.add("Hello 🌍🚀✨");
      // Mixed unicode with emoji
      sequence.add("日本語 🎌 and 中文 🇨🇳");
      // More complex emoji sequences
      sequence.add("👨‍👩‍👧‍👦 Family 👍🏽 Skin tone");
      // Mathematical symbols
      sequence.add("∑∫∂∆∇∞");

      assertEquals(8, sequence.size());
      assertEquals("你好世界", sequence.getAsString(0));
      assertEquals("こんにちは世界", sequence.getAsString(1));
      assertEquals("안녕하세요", sequence.getAsString(2));
      assertEquals("مرحبا بالعالم", sequence.getAsString(3));
      assertEquals("Hello 🌍🚀✨", sequence.getAsString(4));
      assertEquals("日本語 🎌 and 中文 🇨🇳", sequence.getAsString(5));
      assertEquals("👨‍👩‍👧‍👦 Family 👍🏽 Skin tone", sequence.getAsString(6));
      assertEquals("∑∫∂∆∇∞", sequence.getAsString(7));
   }

   @Test
   public void testUnicodeRemoveAndIterate()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      sequence.add("First 🔥");
      sequence.add("Second 你好");
      sequence.add("Third ✨");
      sequence.add("Fourth 🎉");

      // Test remove with unicode
      sequence.remove(1);

      assertEquals(3, sequence.size());
      assertEquals("First 🔥", sequence.getAsString(0));
      assertEquals("Third ✨", sequence.getAsString(1));
      assertEquals("Fourth 🎉", sequence.getAsString(2));

      // Test iterator with unicode
      String[] expected = {"First 🔥", "Third ✨", "Fourth 🎉"};
      int index = 0;
      for (String str : sequence)
      {
         assertEquals(expected[index++], str);
      }
      assertEquals(3, index);
   }

   @Test
   public void testUnicodeWithAutoCreate()
   {
      IDLWStringSequence sequence = new IDLWStringSequence(INITIAL_CAPACITY);

      sequence.add().append("🌟 Star ").append("⭐ Rating");
      sequence.add().append("价格: ¥").append(100);
      sequence.add().append("Температура: ").append("20°C");

      assertEquals(3, sequence.size());
      assertEquals("🌟 Star ⭐ Rating", sequence.getAsString(0));
      assertEquals("价格: ¥100", sequence.getAsString(1));
      assertEquals("Температура: 20°C", sequence.getAsString(2));
   }

   @Test
   public void testUnicodeSet()
   {
      IDLWStringSequence source = new IDLWStringSequence(INITIAL_CAPACITY);
      source.add("Unicode ✓");
      source.add("中文测试");
      source.add("🎨🎭🎪");

      IDLWStringSequence target = new IDLWStringSequence();
      target.set(source);

      assertEquals(source.size(), target.size());
      assertEquals("Unicode ✓", target.getAsString(0));
      assertEquals("中文测试", target.getAsString(1));
      assertEquals("🎨🎭🎪", target.getAsString(2));
   }
}
